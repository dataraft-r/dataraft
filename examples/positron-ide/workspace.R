...
policy_contract <- dr_contract(
  ...
  key = "policy_id",
  rules = list(dr_quality_rule("nonnegative_premium", ~ premium >= 0))
)
policies <- dr_product("policies", policies_sample, contract = policy_contract)
policies_downstream <- dr_product("policies_downstream") |>
  dr_add_source(policies)

# The custom editor opens executable ODCS, not dr_contract_yaml() metadata.
dr_contract_odcs(policy_contract, "policies.odcs.yaml")
cat(
