...
# dataraft.core::dr_review() opens a bounded diagnostic table using the IDE data viewer.
if (interactive()) {
  dataraft.core::dr_review(checked, rule = "nonnegative_premium", limit = 20)
}
...
published <- policy_product |>
  dataraft.core::dr_add_source(good_delivery) |>
  dr_set_target(dataraft.adapters::dr_target_rds(release_directory)) |>
  dr_publish()

pinned_source <- dataraft.adapters::dr_source_rds(
  release_directory,
  published$outputs$version
)
stopifnot(identical(
  dataraft.core::dr_read_source(pinned_source)$premium,
  c(120, 80, 100)
))
