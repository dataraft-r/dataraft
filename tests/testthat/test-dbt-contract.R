...
test_that("quality engines and composite keys are never silently discarded", {
  schema <- dr_contract(
    ...
    key = c("id", "amount"),
    rules = list(
      dr_quality_rule("positive", ~ amount > 0),
      dr_pointblank_checks("external", function(data) data)
    )
  )
  expect_error(
...
test_that("SQL type mappings are explicit and drafts cannot enforce schemas", {
  schema <- dr_contract(columns = c(id = "integer64", details = "list"))
  expect_error(
  ...
  draft <- dr_contract_from(data.frame(id = 1L), "draft")
