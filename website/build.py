...
dr_collect(result)'''
home='''<section class="hero-rebuilt" aria-labelledby="home-headline"><div class="hero-message"><span class="hero-kicker">DATARAFT / FOR TEAMS BUILDING REPORTS IN R</span><h1 id="home-headline">Catch bad data before it reaches your report.</h1><p class="hero-intro">Define what each delivery should contain. Check it against your rules. Keep the accepted version and the evidence behind it. DataRaft brings these steps together in R, so you can explain a reported number later.</p><a class="button hero-action" href="/about/">See the monthly report example <span aria-hidden="true">↗</span></a><p class="hero-cue">Plain English walkthrough · No R knowledge needed</p></div><div class="hero-scene" aria-label="Illustrative report workflow: a duplicate contract ID blocks a delivery; after correction it passes the checks"><div class="scene-top"><span>MONTHLY CANCELLATION REPORT</span><span>01 / DATA DELIVERY</span></div><div class="scene-file"><div class="scene-file-head"><span class="scene-file-icon" aria-hidden="true">▦</span><div><strong>contracts.csv</strong><small>New data for this month</small></div></div><div class="scene-columns"><span>contract_id</span><span>status</span><span>month</span></div><div class="scene-row"><span>1042</span><span>active</span><span>Sep</span></div><div class="scene-row scene-error"><span>1042</span><span>cancelled</span><span>Sep</span></div></div><div class="scene-check"><span class="scene-check-symbol" aria-hidden="true">!</span><div><strong>Delivery blocked</strong><span>The same contract ID appears twice.</span></div><span class="scene-check-tag">CHECK</span></div><div class="scene-outcome"><span class="scene-outcome-mark" aria-hidden="true">✓</span><span>Correct the file, check it again, then save the accepted version.</span></div><p class="scene-caption">Illustrative example. You define the rules and configure where accepted data is saved.</p></div></section><section class="section home-explainer"><div class="section-heading"><div><span class="eyebrow">WHY DATARAFT?</span><h2>A repeatable way to work with data.</h2></div><p>A report can change when its input, checks or calculation change. DataRaft keeps those decisions visible.</p></div><div class="principles"><div><span>01 / DESCRIBE</span><h3>Say what you expect.</h3><p>A data contract describes the fields your data should have. You decide the business rules.</p></div><div><span>02 / CHECK</span><h3>Spot problems early.</h3><p>Run quality checks on each delivery. Inspect a rejected result and correct it.</p></div><div><span>03 / REMEMBER</span><h3>Explain the result later.</h3><p>Save an accepted version with the chosen storage tool and keep evidence about the run.</p></div></div><p class="explain-link">DataRaft calls this reusable definition a <strong>data product</strong>. It brings the rules, checks and delivery steps together. <a href="/about/">Follow the full example and meet each package ↗</a></p></section><section class="section"><div class="section-heading"><div><span class="eyebrow">CHOOSE YOUR START</span><h2>Begin where you are.</h2></div><p>You can understand the idea before writing any R code.</p></div>'''+cards([
