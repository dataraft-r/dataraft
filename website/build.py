...
ext='''<p class="lead">Your data products, in view. Inspect contracts, quality evidence and lineage in Positron. Work with contract YAML and offline metadata in VS Code.</p><div class="meta-row"><span>Extension 0.1.0</span><span>Optional integration</span><a href="https://github.com/dataraft-r/dataraft-positron">Extension source ↗</a></div><figure class="screenshot"><a href="/assets/lineage-12-products-dark.png"><img src="/assets/lineage-12-products-dark.png" width="1088" height="768" alt="DataRaft running in Positron: product and quality trees, directed lineage, contract YAML editor and the R session" loading="lazy"></a><figcaption>DataRaft in the native Positron extension host. Synthetic test workspace, dark theme. Click to see the original screenshot.</figcaption></figure><h2>Choose your editor</h2><table><thead><tr><th>Capability</th><th>Positron</th><th>VS Code</th></tr></thead><tbody><tr><td>ODCS contract YAML editor</td><td>Yes</td><td>Yes</td></tr><tr><td>Open metadata JSON snapshots</td><td>Yes</td><td>Yes</td></tr><tr><td>Inspect an existing R session</td><td>With dataraft.ide</td><td>Not supported</td></tr><tr><td>Explicit Trial and bounded R viewer</td><td>With an active, trusted R session</td><td>Not supported</td></tr><tr><td>Verified R rule diagnostics</td><td>With diagnostics-capable bridge</td><td>Not supported</td></tr></tbody></table>'''
s=(UP/'dataraft-positron/README.md').read_text();s=s[s.index('## Install'):];ext+=md(s,'dataraft-positron')
ext+='''<h2>Screenshots from the native application</h2><p>These captures come from the successful main-branch CI run on 23 September 2026. They show the actual extension in Positron, not a mockup of VS Code. The lineage gallery uses synthetic product metadata.</p>'''
for name,title,caption in [('rule-diagnostic.png','Rule diagnostics','A retained trial result maps a failed function rule to its verified R source location.'),('bounded-data-explorer.png','Bounded data preview','The native R Data Explorer displays a bounded selection; data cells do not pass through the extension.'),('lineage-12-products-light.png','Light theme','The same extension host with its product tree, lineage panel and ODCS editor in the native light theme.')]:
 ext+=f'<h3>{title}</h3><figure class="screenshot"><a href="/assets/{name}"><img src="/assets/{name}" width="1088" height="768" loading="lazy" alt="{esc(caption)}"></a><figcaption>{caption}</figcaption></figure>'
ext+='<p><a href="https://github.com/dataraft-r/dataraft-positron/actions/runs/35847997197">Screenshot provenance and complete CI evidence ↗</a></p>'
add('/extension/','DataRaft for Positron & VS Code',ext,'Extension',source=('dataraft-positron','README.md'))
...
def side(url,section):
 ...
 if section=='Get started':links+='<span class="side-label">YOUR FIRST PRODUCT</span>'+''.join(f'<a class="{"active" if url==f"/start/{i+1}/" else ""}" href="/start/{i+1}/">{i+1}. {t}</a>' for i,t in enumerate(start_titles))
