...
# lake for DuckLake; ide for the extension; dbt for dbt. Catalog integrations live in adapters. Exact
...
# 1.11.0. The current course uses DataRaft 0.1.0.9006; exact SHAs appear below. An earlier base test
# also used R 4.4.3. Translation preserves the executable exercises apart from English display
# labels and messages. The current API cleanup was rechecked locally with R 4.3.3: H01-H38
# passed, including DuckLake. H39 requires Shiny/bslib; editor and external-service exercises
# remain explicitly opt-in. CI separately exercises the package and editor integration suites.
...
training_options <- utils::modifyList(
  list(
    lake = FALSE,
    extras = FALSE,
    gui = FALSE,
...
training_step <- function(
  ...
  if (is.null(reason) && length(needs)) {
    missing <- needs[
      !vapply(needs, function(x) identical(training_log[[x]], "ok"), logical(1))
    ]
    if (length(missing)) {
      reason <- paste("Missing prerequisite:", paste(missing, collapse = ", "))
    }
  }
  if (is.null(reason) && length(packages)) {
    ...
    if (length(missing)) {
      reason <- paste("Missing package:", paste(missing, collapse = ", "))
    }
  }
  if (!is.null(reason)) {
  ...
  tryCatch(
    ...
    error = function(e) {
      ...
      if (!optional) {
        stop(id, ": ", conditionMessage(e), call. = FALSE)
      }
      message(id, " | ", training_log[[id]])
    }
  )
...
training_step(
  "H05",
  {
    policy_product <- dr_add_quality(
      policy_product,
      list(nonnegative = ~ premium >= 0)
    )
    policy_run <- dr_run(policy_product, write = FALSE)
...
training_step(
  "H06",
  {
    ...
    stopifnot(
      ...
      last_failure$run_id == failed_run$run_id
    )
  },
  needs = c("H05")
)
...
training_step(
  "H08",
  {
    rounding_recipe <- dr_recipe() |>
      dr_step_mutate(premium = round(premium, 2))
...
training_step(
  "H11",
  {
    ...
    small_contract <- dr_contract(
      "policy.amounts",
      c(policy_id = "integer", premium = "numeric"),
      key = "policy_id"
    )
    small_product <- dr_product(
      "policy.amounts",
      policies,
      contract = small_contract
    ) |>
      dr_add_recipe(small_recipe)
...
training_step(
  "H14",
  {
    rds_source <- dataraft.adapters::dr_source_rds(
      rds_path,
      version = rds_first$outputs$version
    )
    reloaded_product <- dr_product(
      "policies.reloaded",
      rds_source,
...
training_step(
  "H15",
  {
    ...
    latest_rows <- dr_collect(dr_run(
      dr_product("latest", latest_source),
      write = FALSE
    ))
...
training_step(
  "H16",
  {
    ...
    customer_recipe <- dr_recipe() |>
      dr_step_lookup(customers, by = "customer_id")
    customer_product <- dr_product(
      "policies.linked",
      linked_policies,
      contract = linked_contract
    ) |>
      dr_add_recipe(customer_recipe)
...
training_step(
  "H17",
  {
    brokers <- data.frame(
      broker_id = 1:2,
      broker_name = c("Broker A", "Broker B")
    )
    enriched_contract <- dr_contract_update(
      linked_contract,
      version = "2.0.0",
      columns = c(broker_name = "character")
    )
    enrichment_recipe <- customer_recipe |>
      dr_step_lookup(brokers, by = "broker_id")
    enriched_product <- dr_product(
      "policies.enriched",
      linked_policies,
      contract = enriched_contract
    ) |>
      dr_add_recipe(enrichment_recipe)
...
training_step(
  "H19",
  {
    ...
    orphan_run <- dr_run(
      model_product,
      sources = list(customers = customers[1:2, ]),
      write = FALSE,
      stop_on_failure = FALSE
    )
...
training_step(
  "H21",
  {
    cohort_recipe <- dr_recipe() |>
      dr_step_mutate(
        in_opening = start_date <= period_start &
          (is.na(cancel_date) | cancel_date > period_start)
      ) |>
      dr_step_mutate(
        in_cancellations = in_opening &
          !is.na(cancel_date) &
          cancel_date <= period_end
      )
    cohort_contract <- dr_contract(
      "cohort",
      c(
        policy_id = "integer",
        premium = "numeric",
    ...
    cohort_product <- dr_product(
      "policies.cohort",
      dated_policies,
      contract = cohort_contract
    ) |>
      dr_add_recipe(cohort_recipe) |>
      dr_add_quality(list(nonempty_denominator = ~ sum(in_opening) > 0))
    cohort_run <- dr_run(cohort_product, write = FALSE)
    cohort_rows <- dr_collect(cohort_run)
    stopifnot(
      sum(cohort_rows$in_opening) == 6L,
      sum(cohort_rows$in_cancellations) == 1L
    )
  },
  needs = c("H21a")
)
...
training_step(
  "H22",
  {
    cancellation_metric <- dataraft.metrics::dr_metric(
      "policies.cancellation_rate",
      "policies.cohort",
      expr = sum(in_cancellations) / sum(in_opening),
      dimensions = "broker_id",
      unit = "ratio",
      approved = FALSE
    )
...
training_step(
  "H23",
  {
    local_rate <- dataraft.metrics::dr_measure(
      cohort_run,
      cancellation_metric,
      by = character()
    )
...
training_step(
  "H23a",
  {
    ...
    no_opening_run <- dr_run(
      cohort_product,
      data = no_opening_data,
      write = FALSE,
      stop_on_failure = FALSE
    )
...
training_step(
  "H24",
  {
    broker_rates <- dataraft.metrics::dr_measure(
      cohort_run,
      cancellation_metric,
      by = "broker_id"
    )
...
training_step(
  "H25",
  {
    lake_config <- dataraft.lake::dr_lake_config(
      path = file.path(training_root, "ducklake"),
      backend = "ducklake",
      install_extensions = training_options$install_extensions
    )
    print(lake_config)
  },
  packages = c("dataraft.lake"),
  enabled = training_options$lake,
  optional = TRUE
)
...
training_step(
  "H27",
  {
    lake_product <- prepared_product |>
      dr_set_target(dataraft.lake::dr_target_lake(training_lake))
...
training_step(
  "H28",
  {
    ...
    first_lake_rows <- dataraft.lake::dr_read_release(
      training_lake,
      "policies",
      release = lake_first$release_id
    )
...
training_step(
  "H29",
  {
    ...
    lake_second <- dr_publish(
      lake_product,
      data = lake_second_data,
      business_date = "2026-08-31"
    )
    lake_difference <- dataraft.lake::dr_compare(
      lake_first,
      lake_second,
      key = "policy_id"
    )
...
training_step(
  "H30",
  {
    ...
    lake_corrected <- dr_publish(
      lake_product,
      data = lake_correction_data,
      previous = lake_second,
      business_date = "2026-08-31"
    )
    print(dataraft.lake::dr_compare(
      lake_second,
      lake_corrected,
      key = "policy_id"
    ))
    stopifnot(
      sum(dr_collect(lake_first)$premium) == 740,
      sum(dr_collect(lake_corrected)$premium) == 745
    )
  },
  needs = c("H29"),
  enabled = training_options$lake,
...
training_step(
  "H31",
  {
    raw_accepted <- dataraft.lake::dr_ingest(
      policies,
      to = training_lake,
      name = "policies.raw",
      contract = policy_contract,
    ...
    raw_rejected <- dataraft.lake::dr_ingest(
      bad_policies,
      to = training_lake,
      name = "policies.raw",
      contract = policy_contract,
    ...
    stopifnot(
      raw_accepted$status == "published",
      raw_rejected$status == "blocked"
    )
...
training_step(
  "H32",
  {
    raw_source <- dataraft.lake::dr_source_release(
      training_lake,
      "policies.raw",
      raw_accepted$release_id
    )
    clean_product <- dr_product(
      "policies.clean",
      raw_source,
      contract = policy_contract
    ) |>
      dr_add_recipe(rounding_recipe)
...
training_step(
  "H33a",
  {
    model_policy_release <- dr_publish(
      dr_product("model.policies", linked_policies),
      to = training_lake
    )
    model_customer_release <- dr_publish(
      dr_product("model.customers", customers),
      to = training_lake
    )
    model_broker_release <- dr_publish(
      dr_product("model.brokers", brokers),
      to = training_lake
    )
    print(c(
      policies = model_policy_release$release_id,
      customers = model_customer_release$release_id,
      brokers = model_broker_release$release_id
    ))
  },
  needs = c("H26", "H18"),
  enabled = training_options$lake,
...
training_step(
  "H34",
  {
    portfolio_metrics <- dataraft.metrics::dr_metric_set(
      "policies.cohort",
      opening = sum(in_opening),
      cancellations = sum(in_cancellations),
      ...
      approved = TRUE,
      code_version = "training-metrics-v1"
    )
    local_metrics <- dataraft.metrics::dr_measure(
      cohort_run,
...
training_step(
  "H35",
  {
    ...
    report_values <- dataraft.metrics::dr_measure(
      cohort_release,
      metrics = portfolio_metrics,
      by = character()
    )
    frozen_report <- dataraft.metrics::dr_report_release(
      report_values,
      id = "training.august.v1",
      code_version = "training-report-v1",
      to = training_lake
    )
    print(frozen_report)
  },
...
training_step(
  "H36",
  {
    report_before <- dataraft.metrics::dr_report_read(
      training_lake,
      "training.august.v1",
      values_only = TRUE
    )
    ...
    cohort_later <- dr_publish(
      cohort_product,
      data = changed_dates,
      to = training_lake
    )
    report_after <- dataraft.metrics::dr_report_read(
      training_lake,
      "training.august.v1",
      values_only = TRUE
    )
    stopifnot(identical(report_before, report_after))
    report_verification <- dataraft.metrics::dr_report_verify(
      training_lake,
      "training.august.v1",
      metrics = portfolio_metrics
    )
    ...
    stopifnot(
      all(report_verification$integrity == "match"),
      all(report_verification$replay == "match")
    )
  },
  needs = c("H35"),
  enabled = training_options$lake,
...
training_step(
  "H38",
  {
    ...
    stopifnot(
      nrow(dr_run_history(evidence_path)) == 2L,
      evidence_bad$status == "blocked"
    )
  },
  needs = c("H06")
)
...
training_step(
  "H39",
  {
    catalog_app <- dataraft.adapters::dr_catalog_app(
      training_lake,
      launch = FALSE
    )
  ...
  needs = c("H26"),
  packages = c("dataraft.adapters", "shiny", "bslib"),
  enabled = training_options$lake,
  optional = TRUE
)
...
training_step(
  "V01",
  {
    contract_draft <- dr_contract_from(
      policies,
      "policies.draft",
      owner = "Training",
      grain = "One policy",
  ...
  needs = c("H01"),
  enabled = training_options$extras,
  optional = TRUE
)
...
training_step(
  "V02a",
  {
    owned_contract <- policy_contract |>
      dr_contract_meta(
        owner = "Risk Analytics",
        description = "Synthetic training policies",
        grain = "One policy"
      )
...
training_step(
  "V02b",
  {
    flexible_contract <- owned_contract |>
...
training_step(
  "V02",
  {
    contract_v2 <- dr_contract_update(
      owned_contract,
      version = "2.0.0",
      columns = c(channel = "character")
    )
...
training_step(
  "V03a",
  {
    warning_rule <- dr_quality_rule(
      "nonnegative",
      ~ premium >= 0,
      action = "warn",
      threshold = 0
    )
    warning_product <- dr_product(
      "policies.warning",
      bad_policies,
      contract = policy_contract
    ) |>
      dr_add_quality(warning_rule)
...
training_step(
  "V03b",
  {
    quarantine_rule <- dr_quality_rule(
      "nonnegative",
      ~ premium >= 0,
      action = "quarantine",
      threshold = 0
    )
    quarantine_product <- dr_product(
      "policies.quarantine",
      bad_policies,
      contract = policy_contract
    ) |>
      dr_add_quality(quarantine_rule)
...
training_step(
  "V03",
  {
    reference_rule <- dr_quality_reference(
      customers,
      by = "customer_id",
      name = "known_customer"
    )
    reference_product <- dr_product("policies.reference", linked_policies) |>
      dr_add_quality(reference_rule)
...
training_step(
  "V04a",
  {
    ...
    variant_product <- dr_product("policies.variant", policies) |>
      dr_add_recipe(variant_recipe)
...
training_step(
  "V04",
  {
    variant_recipe <- dr_step_transform(dr_recipe(), function(data) {
      dplyr::mutate(data, premium = premium * 1.01)
    })
    variant_product <- dr_product("policies.variant", policies) |>
      dr_add_recipe(variant_recipe)
    print(dr_collect(dr_run(variant_product, write = FALSE)))
...
training_step(
  "V05a",
  {
    ...
    reused_product <- dr_product(
      "policies.reused",
      precise_policies,
      contract = policy_contract
    ) |>
      dr_add_recipe(reused_recipe)
...
training_step(
  "V05",
  {
    replaced_product <- dr_update_recipe(
      reused_product,
      dr_recipe() |> dr_step_mutate(premium = round(premium, 0))
    )
...
training_step(
  "V07a",
  {
    pointblank_rule <- dr_quality_rule("nonnegative", ~ premium >= 0) |>
      dr_set_engine("pointblank")
    pointblank_product <- dr_product(
      "policies.pointblank",
      bad_policies,
      contract = policy_contract
    ) |>
      dr_add_quality(pointblank_rule)
    pointblank_run <- dr_run(
      pointblank_product,
      write = FALSE,
      stop_on_failure = FALSE
    )
...
training_step(
  "V07",
  {
    agent_rule <- dr_pointblank_checks(
      "premium_agent",
      function(data) {
        pointblank::create_agent(data) |>
          pointblank::col_vals_gte(columns = "premium", value = 0)
      },
      action = "block",
      threshold = 0
    )
    agent_product <- dr_product(
      "policies.agent",
      bad_policies,
      contract = policy_contract
    ) |>
      dr_add_quality(agent_rule)
    agent_run <- dr_run(agent_product, write = FALSE, stop_on_failure = FALSE)
    print(dr_quality_report(agent_run))
    agent_contract <- dr_contract(
      "agent.report",
      c(policy_id = "integer", premium = "numeric", cancelled = "logical"),
      rules = list(agent_rule)
    )
    agent_quality <- dr_validate(
      bad_policies,
      agent_contract,
      keep_agents = TRUE
    )
    dr_pointblank_report(
      agent_quality,
      "premium_agent",
      file.path(training_root, "pointblank.html"),
      overwrite = TRUE
    )
  },
  needs = c("V07a"),
...
training_step(
  "V08",
  {
    ...
    xlsx_source <- dr_source_file(
      "policies.excel",
      xlsx_path,
      reader = readxl::read_excel
    )
    xlsx_product <- dr_product("policies.excel", xlsx_source) |>
      dr_add_recipe(
        dr_recipe() |> dr_step_mutate(policy_id = as.integer(policy_id))
      ) |>
      dr_add_contract(policy_contract)
...
training_step(
  "V09a",
  {
    ...
    database_source <- dataraft.adapters::dr_source_database(
      adapter_con,
      table = "policies",
      lazy = FALSE
    )
  ...
  needs = c("H04"),
  packages = c("dataraft.adapters", "RSQLite"),
  enabled = training_options$extras,
  optional = TRUE
)
...
training_step(
  "V09b",
  {
    database_contract <- dr_contract(
      "policies.sqlite",
      c(policy_id = "integer", premium = "numeric", cancelled = "integer"),
      key = "policy_id"
    )
    database_recipe <- dr_recipe() |>
      dr_step_mutate(cancelled = as.integer(cancelled))
    database_product <- dr_product(
      "policies.sqlite",
      policies,
      contract = database_contract
    ) |>
      dr_add_recipe(database_recipe) |>
...
training_step(
  ...
  {
    sql_source <- policies
    sql_recipe <- dr_recipe() |>
      dr_step_transform(dataraft.adapters::dr_sql_transform(
        "SELECT * FROM data WHERE premium >= 100"
      ))
    sql_product <- dr_product("policies.sql", sql_source) |>
      dr_add_recipe(sql_recipe)
...
training_step(
  "V09d",
  {
    ...
    parquet_product <- policy_product |>
      dr_set_target(dataraft.adapters::dr_target_parquet(
        parquet_path,
        overwrite = TRUE
      ))
  ...
  needs = c("H07"),
  packages = c("dataraft.adapters", "arrow"),
  enabled = training_options$extras,
  optional = TRUE
)
...
training_step(
  "V09e",
  {
    ...
    parquet_rows <- dr_collect(dr_run(
      dr_product("policies.parquet", parquet_source),
      write = FALSE
    ))
    print(parquet_rows)
...
training_step(
  "V09f",
  {
    ...
    pins_product <- policy_product |>
      dr_set_target(dataraft.adapters::dr_target_pins(
        training_board,
        "policies"
      ))
  ...
  needs = c("H07"),
  packages = c("dataraft.adapters", "pins"),
  enabled = training_options$extras,
  optional = TRUE
)
...
training_step(
  "V09g",
  {
    ...
    pins_rows <- dr_collect(dr_run(
      dr_product("policies.pin", pins_source),
      write = FALSE
    ))
    print(pins_rows)
...
training_step(
  "V09",
  {
    ...
    api_source <- dataraft.adapters::dr_source_api(
      api_request,
      parse = function(response) {
        dplyr::bind_rows(httr2::resp_body_json(
          response,
  ...
  packages = c("dataraft.adapters", "httr2"),
  enabled = training_options$api,
  optional = TRUE
)
...
training_step(
  "V10a",
  {
    ...
    if (!file.exists(odcs_path)) {
      dataraft.adapters::dr_contract_odcs(policy_contract, path = odcs_path)
    }
    cat(
      readLines(odcs_path),
      sep = "
  ...
  needs = c("H04"),
  packages = c("dataraft.adapters", "yaml"),
  enabled = training_options$extras || training_options$gui,
  optional = TRUE
)
...
training_step(
  "V11c",
  {
    ...
    lineage_catalog <- dataraft.adapters::dr_catalog_openlineage(
      lineage_endpoint,
      namespace = "dataraft-training"
    )
    lineage_product <- policy_product |> dr_add_catalog(lineage_catalog)
    lineage_run <- dr_run(
      lineage_product,
      evidence = file.path(training_root, "lineage-evidence")
    )
    print(lineage_run)
  },
  needs = c("H07"),
  packages = c("dataraft.adapters", "httr2"),
  enabled = training_options$catalogs,
  optional = TRUE
)
...
training_step(
  "V11d",
  {
    ...
    if (nzchar(Sys.getenv("TRAINING_OPENMETADATA_TOKEN"))) {
      om_request <- httr2::req_auth_bearer_token(
        om_request,
        Sys.getenv("TRAINING_OPENMETADATA_TOKEN")
      )
    }
    om_catalog <- dataraft.adapters::dr_catalog_openmetadata(
      om_url,
      database_schema = om_schema,
      request = om_request
    )
    om_run <- dr_run(
      dr_add_catalog(policy_product, om_catalog),
      evidence = file.path(training_root, "om-evidence")
    )
    print(om_run)
  },
  needs = c("H07"),
...
training_step(
  "V11",
  {
    retry_history <- dr_retry_catalogs(
      file.path(training_root, "lineage-evidence"),
      list(lineage_catalog)
    )
    print(retry_history)
  },
  needs = c("V11c"),
...
training_step(
  ...
  {
    stopifnot(utils::packageVersion("duckdb") >= "1.5.5")
    dbt_config <- dataraft.lake::dr_lake_config(
      path = file.path(training_root, "dbt-lake"),
      backend = "duckdb",
      layers = c("raw", "staging", "core", "marts")
    )
    dbt_input <- data.frame(
      order_id = linked_policies$policy_id,
      customer_id = linked_policies$customer_id,
      amount = linked_policies$premium
    )
    dbt_raw <- dataraft.lake::dr_ingest(
      dbt_input,
      to = dbt_config,
  ...
  needs = c("H16a"),
  packages = c("dataraft.dbt", "dataraft.lake", "duckdb", "bit64", "yaml"),
  enabled = training_options$dbt,
  optional = TRUE
)
...
training_step(
  "V12",
  {
    ...
    dbt_project <- dataraft.dbt::dr_dbt_init(
      file.path(training_root, "dbt-project"),
      dbt_config,
      name = "training_policies",
      executable = dbt_executable,
      sources = list(orders = dbt_raw)
    )
    dbt_project <- dataraft.dbt::dr_dbt_sources(
      dbt_project,
      list(orders = dbt_raw)
    )
    print(dbt_project)
...
training_step(
  "V13a",
  {
    dbt_result <- dataraft.dbt::dr_dbt_build(
      dbt_project,
      echo = TRUE,
      stop_on_failure = FALSE
    )
...
training_step(
  "V14a",
  {
    ...
    dbt_dm <- dataraft.dbt::dr_dbt_model(
      dbt_lake,
      dbt_result,
      tables = c(premiums = dbt_model_id),
      primary_keys = list(premiums = "customer_id")
    )
...
training_step(
  "V14b",
  {
    dbt_output_contract <- dr_contract_from(
      premium_totals,
      "customer.premiums",
      grain = "One customer",
      key = "customer_id",
      required = c("customer_id", "revenue")
    ) |>
      dr_contract_confirm()
    dbt_contract_spec <- dataraft.dbt::dr_dbt_contract(
      dbt_output_contract,
...
training_step(
  "V14",
  {
    dbt_release <- dataraft.dbt::dr_dbt_publish(
      dbt_lake,
      dbt_result,
      dbt_model_id,
      contract = dbt_output_contract,
      asset = "customer.premiums",
      code_version = "training-dbt-v1"
    )
...
training_step(
  "V14c",
  {
    manifest_contract <- tryCatch(
      dataraft.dbt::dr_dbt_contract_from_manifest(
        dbt_result,
        "model.training_policies.customer_revenue"
      ),
      error = identity
    )
...
training_step(
  "V14d",
  {
    ...
    stopifnot(
      nzchar(transform_path),
      nzchar(transform_model),
      nzchar(transform_db)
    )
    transform_factory <- function() {
      DBI::dbConnect(duckdb::duckdb(), transform_db)
    }
    {
      transform_project <- dataraft.dbt::dr_dbt_project(
        transform_path,
        profiles_dir = transform_path,
        executable = Sys.getenv("TRAINING_DBT_EXECUTABLE", "dbt")
      )
      dbt_transform <- dataraft.dbt::dr_transform_dbt(
        transform_project,
        model = transform_model,
        connection = transform_factory,
        input = "training_input"
      )
      print(dr_collect(dr_run(
        dr_product("policies.dbt.transform", policies) |>
          dr_add_recipe(dr_recipe() |> dr_step_transform(dbt_transform)),
        write = FALSE
      )))
    }
  },
  packages = c("dataraft.dbt", "duckdb", "processx"),
  enabled = training_options$dbt &&
    nzchar(Sys.getenv("TRAINING_DBT_TRANSFORM_PROJECT")),
  optional = TRUE
)
...
training_step(
  "V14e",
  {
    ...
    stopifnot(
      nzchar(om_dbt_url),
      nzchar(om_service),
      nzchar(Sys.getenv("OPENMETADATA_JWT_TOKEN"))
    )
    om_dbt <- dataraft.adapters::dr_catalog_openmetadata_dbt(
      om_dbt_url,
      service = om_service,
  ...
  needs = c("V13a"),
  packages = c("dataraft.adapters", "processx"),
  enabled = training_options$catalogs && training_options$dbt,
  optional = TRUE
)
  ...
  "V15",
  {
    training_flow <- dr_workflow(
      checked = function(delivery) {
        dr_run(policy_product, data = delivery, write = FALSE)
      },
      total = function(checked) sum(dr_collect(checked)$premium),
      inputs = list(delivery = policies),
      code_version = "training-flow-v1"
    )
...
training_step(
  "V16",
  {
    ...
    flow_second <- dr_run(
      training_flow,
      inputs = list(delivery = flow_corrected_data),
      previous = flow_first
    )
...
training_step(
  "V17b",
  {
    ...
    yaml::write_yaml(
      list(
        id = "policies.fileproject",
        contract = "policies.contract.yaml",
        source = "policies.csv",
        target = "yaml-rds"
      ),
      project_yaml_path
  ...
  needs = c("V10a", "V08a"),
  packages = c("yaml", "dataraft.adapters"),
  enabled = training_options$extras,
  optional = TRUE
)
...
training_step(
  ...
  needs = c("H07"),
  packages = c("dataraft.adapters", "targets"),
  enabled = training_options$extras,
  optional = TRUE
)
...
training_step(
  "V18a",
  {
    ...
    pg_catalog <- dataraft.lake::dr_registry_postgres(
      "TRAINING_PG_CONNECTION",
      lock_timeout = 10
    )
    pg_config <- dataraft.lake::dr_lake_config(
      catalog = pg_catalog,
      storage = dataraft.lake::dr_storage_local(file.path(
        training_root,
        "pg-data"
      )),
      landing = file.path(training_root, "pg-landing"),
      backend = "ducklake",
      install_extensions = training_options$install_extensions
    )
    print(pg_config)
  },
  packages = c("dataraft.lake", "RPostgres", "duckdb", "bit64"),
  enabled = training_options$postgres,
  optional = TRUE
)
...
training_step(
  "V18",
  {
    pg_release <- dr_publish(
      dr_product("training.pg.policies", policies, contract = policy_contract),
      to = pg_config
    )
    print(pg_release)
...
training_step(
  "V19a",
  {
    ...
    s3_storage <- dataraft.lake::dr_storage_s3(
      s3_bucket,
      prefix = paste0("dataraft-training/", basename(training_root)),
      endpoint = s3_endpoint,
      credential_provider = "credential_chain"
    )
    s3_config <- dataraft.lake::dr_lake_config(
      catalog = dataraft.lake::dr_registry_duckdb(file.path(
        training_root,
        "s3-metadata.ducklake"
      )),
      storage = s3_storage,
      landing = file.path(training_root, "s3-landing"),
      backend = "ducklake",
      install_extensions = training_options$install_extensions
    )
    print(s3_config)
  },
  packages = c("dataraft.lake", "duckdb", "bit64", "paws.storage"),
  enabled = training_options$s3,
  optional = TRUE
)
...
training_step(
  "V19",
  {
    s3_release <- dr_publish(
      dr_product("training.s3.policies", policies, contract = policy_contract),
      to = s3_config
    )
...
training_step(
  "V20",
  {
    ...
    writer_code <- c(
      ...
      'writer <- args[2]',
      'data <- data.frame(policy_id=1L, premium=as.numeric(args[3]), cancelled=FALSE)',
      'product <- dataraft.core::dr_product(paste0("training.writer.", writer), data)',
      'print(dataraft.core::dr_publish(product, to=config))'
    )
...
training_step(
  "V21a",
  {
    delivery_check <- dataraft.lake::dr_check_delivery(
      training_lake,
      "policies",
      policy_contract,
      business_date = as.Date("2026-08-31"),
      due_at = as.POSIXct("2026-09-01 12:00:00", tz = "UTC"),
      at = as.POSIXct("2026-09-02 12:00:00", tz = "UTC"),
      record = FALSE
    )
...
training_step(
  "V21d",
  {
    print(dataraft.lake::dr_cleanup(
      training_lake,
      older_than_days = 30,
      dry_run = TRUE
    ))
...
training_step(
  "V21",
  {
    print(dataraft.lake::dr_expire_snapshots(
      training_lake,
      older_than_days = 30,
...
training_step(
  "V22a",
  {
    ...
    stopifnot(
      !is.null(iceberg_con),
      DBI::dbIsValid(iceberg_con),
      nzchar(iceberg_path)
    )
    iceberg_source <- dataraft.adapters::dr_source_iceberg(
      iceberg_con,
      iceberg_path
    )
    print(dr_collect(dr_run(
      dr_product("training.iceberg", iceberg_source),
      write = FALSE
    )))
  },
  packages = c("dataraft.adapters", "duckdb"),
  enabled = training_options$iceberg,
  optional = TRUE
)
...
training_step(
  "V22",
  {
    ...
    iceberg_target <- dataraft.adapters::dr_target_iceberg(
      iceberg_con,
      DBI::Id(
        catalog = iceberg_catalog,
        schema = iceberg_schema,
        table = "dataraft_training_policies"
      ),
      mode = "create"
    )
...
training_step(
  "V23a",
  {
    training_source <- structure(
      list(data = policies),
      class = "training_source"
    )
    dr_check_component.training_source <- function(x, ...) invisible(x)
    dr_read_source.training_source <- function(source, ...) source$data
    dr_inspect.training_source <- function(x, ...) {
      list(type = "training_source")
    }
    dr_capabilities.training_source <- function(x, ...) {
      dr_component_capabilities(read = TRUE)
    }
    for (generic in c(
      "dr_check_component",
      "dr_read_source",
      ...
      registerS3method(
        generic,
        "training_source",
        get(paste0(generic, ".training_source")),
...
training_step(
  "V23b",
  {
    custom_product <- dr_product(
      "policies.custom",
      training_source,
      contract = policy_contract
    )
    print(dr_collect(dr_run(custom_product, write = FALSE)))
    print(dataraft.adapters::dr_test_adapter(
      training_source,
      expected = policies
    ))
  },
...
training_step(
  "V23",
  {
    ...
    conformance <- dataraft.adapters::dr_test_adapter(
      dataraft.adapters::dr_target_rds(conformance_path),
      data = policies,
      context = list(contract = policy_contract),
      read_back = function(output) {
        dr_read_source(dataraft.adapters::dr_source_rds(
          conformance_path,
          output$version
        ))
      }
...
training_step(
  "V24a",
  {
    column_recipe <- dr_recipe() |>
      dr_step_mutate(annual_premium = premium * 12) |>
      dr_step_select(policy_id, annual_premium)
...
training_step(
  ...
  {
    if (interactive()) {
      dr_review(failed_run, what = "rows", limit = 10)
    }
    if (interactive()) dr_review(failed_run, what = "report")
  },
  needs = c("H06"),
  enabled = training_options$gui,
  optional = TRUE
)
...
training_step(
  ...
  {
    stopifnot(utils::packageVersion("duckdb") >= "1.5.5")
    offline_lake <- dataraft.lake::dr_open_lake(
      file.path(training_root, "offline-duckdb"),
      backend = "duckdb"
    )
    tryCatch(
      {
  ...
  needs = c("H07"),
  packages = c("dataraft.lake", "duckdb", "bit64"),
  enabled = training_options$extras,
  optional = TRUE
)
...
training_step(
  "E04",
  {
    ...
    bridge_context <- dataraft.ide::ide_context(
      workspace = environment(),
      response_root = bridge_root,
      contract_root = training_root
    )
    request <- list(
      version = 1L,
      operation = "products",
      request_id = "training-products",
      response_path = metadata_path
    )
    encoded_request <- jsonlite::base64_enc(charToRaw(as.character(jsonlite::toJSON(
      request,
      auto_unbox = TRUE
    ))))
    dataraft.ide::ide_request(
      gsub("[[:space:]]", "", encoded_request),
      bridge_context
    )
    print(metadata_path)
  },
...
training_step(
  "E15",
  {
    ...
    writeLines(
      c("training_nonnegative <- function(data) {", "  data$premium >= 0", "}"),
      rule_path
    )
    source(rule_path, local = environment(), keep.source = TRUE)
    source_rule_product <- dr_product(
      "policies.source.rule",
      bad_policies,
      contract = policy_contract
    ) |>
      dr_add_quality(list(nonnegative = training_nonnegative))
    print(rule_path)
  },
  needs = c("E01", "H06"),
  enabled = training_options$gui,
...
training_summary <- data.frame(
  ...
  status = vapply(
    sort(ls(training_log)),
    function(id) training_log[[id]],
    character(1)
  ),
  row.names = NULL
)
...
if (exists("adapter_con", inherits = FALSE) && DBI::dbIsValid(adapter_con)) {
  DBI::dbDisconnect(adapter_con)
}
if (
  exists("training_lake", inherits = FALSE) && DBI::dbIsValid(training_lake$con)
) {
  dataraft.lake::dr_disconnect_lake(training_lake)
}
if (exists("dbt_lake", inherits = FALSE) && DBI::dbIsValid(dbt_lake$con)) {
  dataraft.lake::dr_disconnect_lake(dbt_lake)
}
...
# https://github.com/dataraft-r/dataraft.metrics/tree/abc97b130c5a62bd664a45b1ed656a1627d8715d
# dataraft.adapters: 946e037d5e93835bc9b516d85b3dc1df50f2f032
# https://github.com/dataraft-r/dataraft.adapters/tree/946e037d5e93835bc9b516d85b3dc1df50f2f032
# dataraft.dbt: 6c5d1376ce8987dd8166b14415a32090b4df8fae
